...
6. **Start a PRD-to-Issues workspace** using `mcp__vibe_kanban__start_workspace`. First resolve the worktree repo: call `mcp__vibe_kanban__list_repos` and find the repo named `"core-worktrees"` (case-insensitive); fall back to `"core"` if not found.
